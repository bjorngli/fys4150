...
popt,pcov = scipy.curve_fit(func,  matrix_dim,  iterations,  p0=(0, 0,0))
a,b,c = popt

plt.plot(matrix_dim,iterations,'b',zorder=2,alpha=0.4)
plt.plot(matrix_dim,func(matrix_dim,*popt),'r')

plt.title('Iterations needed as a function of matrix dimension')
plt.legend(['Recorded number of iterations','Fitted curve {:4.0f}*exp({:0.4f}x) {:4.0f}'.format(a,b,c)])

...
contains
  ...
  subroutine kinetic_energy(sys,numbodies,kin)
    implicit none
    class(solver), intent(in)::sys
    integer,intent(in)::Numbodies
    integer::i
    real(8),dimension(numbodies+1)::kin
    real(8)::totalke

    totalke=0

    do i =1,numbodies
       kin(i)=(1.d0/2.d0)*sys%mass(i)*(sys%velocity(i,1)**2+sys%velocity(i,2)**2+sys%velocity(i,3)**2)
       totalke=totalke+kin(i)
    end do

    kin(numbodies+1)=totalke
  ...
  subroutine angular_momentum(sys,numbodies,relpos,angular)
    implicit none
    class(solver),intent(in)::sys
    integer,intent(in)::Numbodies
    integer::i,j
    real(8),dimension(numbodies+1)::angular
    real(8),dimension(numbodies,numbodies,4)::relpos
    real(8)::totalang

    totalang=0

    do i=1,numbodies
       angular(i)=sys%mass(i)*relpos(1,i,4)*(sqrt(sys%velocity(i,1)**2.d0 + sys%velocity(i,2)**2.d0 + sys%velocity(i,3)**2.d0))

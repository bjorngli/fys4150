...
PROGRAM ising2dim
  ...
  USE mpi_constants
  USE f90lib
  INTEGER :: n_spins, de, t, tstep, i, cycles, idum, count, total_count
  ...
  !This program calculates how energy and mean magnetization
  !develops for an increasing number of MC cycles
  !It also calculates fraction of accepted configurrations for T=1 and T=2.4
  !(decided in input.dat)
  ...
  IF ( my_rank == 0 ) THEN
     OPEN(UNIT=8,FILE='count1.dat') !Changing the name of this when changing the temperature
     OPEN(UNIT=7,FILE='random.dat') !Changing the name of this when changing to an all spins up lattice
  ...
  DO t = 1, tstep
    DO i=1,mcs,50
       ...
       total_average = 0.0_dp; local_average  =0.0_dp; total_count = 0; count=0
       CALL Metropolis(i, n_spins, my_rank, w, local_average, spin_matrix,count)
       ! collect all results
       ...
       ! print results
       IF (my_rank == 0) THEN
         CALL WritetoFile(n_spins, i*numprocs, temperature, total_average)
         WRITE(8,*) i*numprocs, total_count
       ENDIF
     ENDDO
     ! update temp
...
!
SUBROUTINE  Metropolis(mcs, n_spins, my_rank, w, average, spin_matrix,count)
  USE constants
  IMPLICIT NONE
  INTEGER, INTENT(in) :: n_spins, my_rank, mcs
  INTEGER :: spins, ix, iy,  deltae, right, left, up, down, allspins, cycles, count
  REAL(dp) :: e, m
...
SUBROUTINE WritetoFile(n_spins, mcs, temperature, average)
  ...
  WRITE(7,'(6F12.6)')temperature, Eaverage/n_spins/n_spins, Mabsaverage/n_spins/n_spins

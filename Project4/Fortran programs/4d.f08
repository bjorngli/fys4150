...
PROGRAM ising2dim
  ...
  USE mpi_constants
  USE f90lib
  INTEGER :: n_spins, de, t, tstep, i, cycles, count, total_count, idum
  INTEGER, ALLOCATABLE, DIMENSION(:,:) :: spin_matrix
  INTEGER, DIMENSION(:), ALLOCATABLE :: e_stored, e_tot
  REAL(dp) :: w(-8:8), local_average(5), total_average(5)
  REAL(DP) :: initial_temp, final_temp, E, M, temp_step, temperature
  REAL(DP) :: time_start, time_end

  !This program is supposed to return the number of times the system reaches a
  !certain energy state and count them. Due to some problems with MPI, this
  !was not successful
  ...
  IF ( my_rank == 0 ) THEN
     !OPEN(UNIT=7,FILE='mc_calc.dat')
     OPEN(UNIT=8,FILE='energy_states.dat')
     OPEN(UNIT=5,FILE='input.dat')

     CALL ReadInput(n_spins, mcs, initial_temp, final_temp, temp_step)
  ENDIF
  ...
  ALLOCATE(spin_matrix(n_spins,n_spins),e_stored((-2*n_spins**2)/4:(2*n_spins**2)/4),e_tot((-2*n_spins**2)/4:(2*n_spins**2)/4))
  idum = -(1+my_rank)
  ...
  DO i=1,n_spins
    DO j=1,n_spins
      spin_matrix(i,j)=1-2*NINT(ran1(idum))
    ENDDO
  ENDDO
  ...
  DO t = 1, tstep
       ...
       total_average = 0.0_dp; local_average  =0.0_dp; total_count = 0; e_stored=0; e_tot=0
       CALL Metropolis(mcs, n_spins, my_rank, w, local_average, spin_matrix, count, e_stored)
       ! collect all results
       CALL  MPI_REDUCE(local_average, total_average, 5, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierror)
       CALL  MPI_REDUCE(count,total_count,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierror)
       CALL  MPI_REDUCE(e_stored,e_tot,1+n_spins**2,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierror)
       ! print results
       IF (my_rank == 0) THEN
         WRITE(8,*) temperature, e_tot
       ENDIF
     ! update temp
     temperature = temperature + temp_step
  ENDDO
  DEALLOCATE(spin_matrix,e_stored,e_tot)
  time_end = MPI_Wtime()
  IF (my_rank == 0) THEN
...
! read in input data
!
SUBROUTINE  ReadInput(n_spins, mcs, initial_temp, final_temp, temp_step)
  USE constants
  IMPLICIT NONE
  ...
  READ(5,*) n_spins, mcs, initial_temp, final_temp, temp_step
...
!
SUBROUTINE  Metropolis(mcs, n_spins, my_rank, w, average, spin_matrix, count, e_stored)
  USE constants
  IMPLICIT NONE
  ...
  INTEGER, INTENT(INOUT) :: spin_matrix(n_spins,n_spins),e_stored(1+n_spins**2)
  REAL(DP) :: RandomNumber(3)
  INTEGER :: i, n, clock
  INTEGER, DIMENSION(:), ALLOCATABLE :: seed
  INTEGER :: idum, count, int_e
  ...
  DO cycles = 1, mcs
     ...
     int_e = INT(e/4)
     e_stored(int_e) = e_stored(int_e) +1
     !write(8,*) cycles, e_stored(int_e)
  ENDDO
  !write(*,*) e_stored
  DEALLOCATE(seed)
...
SUBROUTINE WritetoFile(n_spins, mcs, temperature, average)
  ...
  WRITE(7,'(6F12.6)')temperature, Eaverage/n_spins/n_spins, Evariance/temperature/temperature, &
